...
class PvPCWorld implements IKBObject
{
    ...
    public function __construct(string $lvl, bool $kb, PvPCKnockback $knockback)
    {
        ...
        $this->level = Server::getInstance()->getWorldManager()->getWorldByName($lvl);
    ...
    {
        $this->localizedLevel = $level->getFolderName();
        $this->level = $level;
    }
    ...
    public function canUseKnocback(Player $player1, Player $player2): bool
    {
        ...
        return Utils::areLevelsEqual($player1->getWorld(), $this->level)
            && Utils::areLevelsEqual($player2->getWorld(), $this->level);
    }
