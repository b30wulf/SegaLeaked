...
class PvPCArea implements IKBObject
{
    ...
    private function initLevel(&$level): Level
    {
        ...
        if (is_string($level)) {
            return Server::getInstance()->getWorldManager()->getWorldByName($level);
        }
    ...
    public function toArray(): array
    {
        return [
            ...
            "level" => $this->level !== null ? $this->level->getFolderName() : null
        ];
    }
    ...
    public function canUseKnocback(Player $player1, Player $player2): bool
    {
        ...
        $level = $player1->getWorld();
        if (!Utils::areLevelsEqual($level, $player2->getWorld())
            || !Utils::areLevelsEqual($level, $this->level)) {
            return false;
        }

        return $this->isWithinArea($player1->getPosition()->asVector3())
            && $this->isWithinArea($player2->getPosition()->asVector3());
    }
