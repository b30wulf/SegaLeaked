...
class WorldHandler
{
    ...
    public function getWorld($level)
    {
        if ($level instanceof Level) {
            if (!isset($this->worlds[$levelName = $level->getFolderName()])) {
            ...
            $loaded = true;
            if (!$this->server->getWorldManager()->isWorldLoaded($level)) {
                $loaded = $this->server->getWorldManager()->loadWorld($level);
            }

            if (!$loaded) {
                return null;
            }
            return $this->getWorld($this->server->getWorldManager()->getWorldByName($level));
        }
        return null;
    }
    ...
    public function getWorldKnockback(Player $player1, Player $player2): ?PvPCWorld
    {
        ...
        if(Utils::areLevelsEqual($level = $player1->getWorld(), $player2->getWorld()))
        {
    ...
    {
        $worlds = [];
        $levels = $this->server->getWorldManager()->getWorlds();
